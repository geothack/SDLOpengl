...
Shader::Shader(const std::string path)
{
    ...
    if (!m_is_geometry)
        Create(source.vertex.c_str(), source.fragment.c_str());
    else
        Create(source.vertex.c_str(), source.fragment.c_str(),source.geometry.c_str());
}
...
void Shader::Create(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
    ...
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float)1000 / (float)800, 0.1f, 100.0f);
    glBindBuffer(GL_UNIFORM_BUFFER, uboMatrices);
